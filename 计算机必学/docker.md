...
### 一、镜像（Image）
#### 定义
Docker 镜像本质是**运行应用所需所有依赖（代码、运行时、库、环境变量、配置文件）的静态模板**，可理解为**容器的只读模板/软件安装包**。

#### 核心特点
1. **只读不可变**：镜像创建后无法直接修改，修改需基于原镜像构建新镜像
2. **分层存储**：由多个只读层叠加而成，相同层可共享，节省存储空间
3. **一致性**：一次构建，在任何环境运行结果完全一致

#### 作用
容器的运行基础，**一个镜像可以启动多个相互独立的容器**。

#### 示例
`nginx:1.24`、`mysql:8.0`、`ubuntu:22.04` 都是标准镜像。


### 二、容器（Container）
#### 定义
容器是**镜像的动态运行实例**，是 Docker 核心运行单元，是轻量级、隔离的应用运行环境。

#### 核心特点
1. **可读写层**：在只读镜像之上增加一层可读写层，运行修改仅作用于容器，不影响原镜像
2. **环境隔离**：拥有独立文件系统、网络、进程空间，容器之间互不干扰
3. **生命周期管理**：支持创建、启动、停止、重启、删除等操作

#### 示例
基于 `mysql:8.0` 镜像启动一个容器，就是一个独立运行的 MySQL 数据库服务。

### 三、镜像仓库（Image Registry）
#### 定义
集中**存储、分发、管理 Docker 镜像**的远程服务，相当于**镜像的应用商店**。

#### 核心分类
1. **公共仓库**：公开访问，免费使用
   - 官方：Docker Hub（最常用）
   - 国内：阿里云镜像仓库、网易云镜像
2. **私有仓库**：企业/个人内部使用，保障安全私密
   - 自建：Docker Registry
   - 云服务：阿里云 ACR、腾讯云 TCR

#### 核心作用
- 上传（push）：开发者将构建好的镜像推送到仓库
- 下载（pull）：服务器/测试环境从仓库拉取镜像
- 统一管理：版本控制、团队共享、跨环境部署
